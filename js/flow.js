var ref$, Str, fold1, objToLists, listsToObj, flatten, unique, map, curry, ggl, getY, getM, dragMove, tsv2Json, sumupST, sumupV, sumSex, sumAge, sumYM, sumDs, json2NodeLink, buildSankey, buildMargin, buildSvg, builGenderBar, buildHorizonBar, buildHeatMap;
...
buildHorizonBar = function(data){
  var bar, svg, ttl, sclBar, dt, rct;
  bar = buildMargin();
  svg = buildSvg(bar);
  ttl = fold1(curry$(function(x$, y$){
    return x$ + y$;
  ...
  sclBar = d3.scale.linear().domain([0, ttl]).range([0, bar.w]);
  dt = svg.selectAll(".rect").data(data.sort(function(a, b){
    return b.value - a.value;
  })).each(function(){}).enter();
  rct = dt.append("rect").attr({
    "width": function(it){
      return sclBar(it.value);
    },
    "height": 15,
...
d3.tsv("../transform/group/transfer_t10.tsv", function(err, tsvBody){
  var json, crx, allGrp, sexDim, ageDim, p23Dim, spdDim, cycDim, sexGrp, ageGrp, p23Grp, spdGrp, cycGrp, lsGrp;
  json = tsv2Json(
  tsvBody);
  console.log(
  json);
  ...
  lsGrp = [sexGrp.all(), ageGrp.all(), p23Grp.all(), spdGrp.all(), cycGrp.all()];
  lsGrp.map(function(it){
    return buildHorizonBar(
    it);
  });
  console.log(sexGrp.all());
